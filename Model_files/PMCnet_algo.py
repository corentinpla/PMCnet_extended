...
def SL_PMC_Adapt_Cov_new(N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1, epsilon2):
    ...
    for t in range(T):
        ...
        for s in range(sample_number):
            _,_,nlogf_temp = evaluate_target_general(all_samples[t][:,s].double(),tp) #nlogf_temp : loss with penalty 
            nlogf.append(nlogf_temp)
        logf = -torch.tensor(nlogf)
        #logf = logf-logf.max()+100
        ...
        rob_Cov_2.append(rob_Cov_2_0) # bias covariance matrix of the proposal density for next iteration
        ...
        Past_A=Save_A
        ct_vec.append(ct)
        print(['Iteration ',t,': we discard ',"ct:",ct,'/',N,' Sigma matrices'])
