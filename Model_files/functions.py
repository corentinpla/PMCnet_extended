...
def FFnetwork(W,b,L,X,activation):
    for ll in range(0,L): #L : number of hidden layers 
        # dropout
        p = 1
        pdf = bernoulli(p)
...
def evaluate_proposal_multiple_fullCov(x, mu, Sig,N):
    ...
    #print("sigma_constant",sigma_constant)
...
def BNN_posterior_extended(N, K,population, weights, x_test, y_test1, tp):
    m = nn.Sigmoid()
    L =tp['L']
    weights = np.nan_to_num(weights)
    wn = weights/sum(weights)
    ...
    z_est_test_EV = (expected_value>0.5)*1.0
    matrix_EV=confusion_matrix(np.array(y_test1),np.array(z_est_test_EV.cpu()))
    ...
    Precision_EV = TP/(TP+FP)
    Recall_EV  = TP/(TP+FN)
    Specificity_EV  = TN/(TN+FP)
    Accuracy_EV  = (TP+TN)/(TP+TN+FP+FN)
    F1_EV  = 2*TP/(2*TP+FP+FN)

    metrics_EV={}
    metrics_EV["AUC"]=auc_test_EV
    metrics_EV["tpr"]=tpr_test_EV
    metrics_EV["fpr"]=fpr_test_EV
    metrics_EV["TP"]=TP
    metrics_EV["FN"]=FN
    metrics_EV["FP"]=FP
    metrics_EV["TN"]=TN
    metrics_EV["precision"]=Precision_EV
    metrics_EV["recall"]=Recall_EV
...
def BNN_posterior_multiclass_extended(N, K,population, weights, x_test, y_test1, tp):
    ...
    metrics_EV["FPR"]=fpr_test_particle
    metrics_EV["TPR"]=tpr_test_particle
    metrics_EV["AUCAv"]=aucAv_test_particle
