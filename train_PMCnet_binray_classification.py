...
tp['x_0'] = x_train
tp['y'] = y_train
tp['regularization_weight'] = 1.8540900000000002
...
lr = 1e-3
results_dir = 'results/Ionosphere'  
...
N = 50  # number of proposals #50
K = 200  # samples per proposal per iteration #100
sig_prop = 0.1
lr = 2  #glocal resampling
gr_period=2
tp['regularization_weight'] = 1.8540900000000002 #with extended
epsilon1 = 1e-299
epsilon2 = 1e-299
...
logger = get_logger('log_BNN_Ionosphere_l2.txt') 
...
if dogolden_search == 0:    
    ...
    with open("bug_log_BNN_binaryclass.txt", "w") as log:
        ...
        try:
            for i in range(1): 
                ...
                plt.legend()
                plt.grid(True)
                plt.savefig("/workspace/code/weights_extended_lr=1_sig_prop=0.1.png")
                ...
                path_save_BNN_output  = os.path.join(results_dir,'output_Ionosphere_l2_final.txt')             
    ...
    with open("bug_log_BNN_binaryclass.txt", "w") as log:
        ...
        try:
            crit = lambda reg: run_PMCnet_fixedReg_extended(simulations,reg,loss,results_dir, x_val, y_val1,is_binary,N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1,epsilon2,logger)
            ...
            if dosave == 1:
                path_save_BNN_output  = os.path.join(results_dir,'final_output_reg_Ionosphere_l2_'+str(reg1)+'.txt')             
                with open(path_save_BNN_output, "wb") as fp:   #Pickling
                    pickle.dump(reg_list, fp)

                path_save_BNN_output  = os.path.join(results_dir,'final_output_loss_Ionosphere_l2_'+str(reg1)+'.txt')           
                with open(path_save_BNN_output, "wb") as fp:   #Pickling
                    pickle.dump(Loss_list, fp)
            myprint('the final regularization weight is {}'.format(reg_final),logger)
            myprint('the reg list is {}'.format(reg_list),logger)  
            myprint('the loss lilst is {}'.format(Loss_list), logger )
