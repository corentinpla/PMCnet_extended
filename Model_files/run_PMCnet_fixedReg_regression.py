...
def run_PMCnet_fixedReg_regression(simulations,reg,loss,N_resampled, results_dir, x_val, y_val,is_binary, N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1,epsilon2,logger):
    ...
    path_save_BNN_output  = os.path.join(results_dir,'output_autoMPG_l2_'+str(reg1)+'.txt')             
    with open(path_save_BNN_output, "wb") as fp:   #Pickling
        pickle.dump(output_vec, fp)
    path_save_BNN_output_posterior  = os.path.join(results_dir,'output_posterior_val_autoMPG_l2_'+str(reg1)+'.txt')             
...
def run_PMCnet_fixedReg_regression_extended(simulations,reg,loss, results_dir, x_val, y_val,is_binary, N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1,epsilon2,logger):
    print("yes it run")
    tp['regularization_weight'] = reg
    output_vec = []
    output_posterior_val_vec = []
    ...
    for i in range(simulations): 
        ...
        population = all_samples_temp[-1] # the last iteration
        weights = all_weights_temp[-1]
        output_posterior_val = BNN_posterior_regression_extended(N, K,population, weights, x_val, y_val, tp)
        output_posterior_val_vec.append(output_posterior_val)
        MSE_mean = np.mean(output_posterior_val[-2])
        MSE_mean_vec.append(MSE_mean)
        myprint(MSE_mean_vec,logger)
        # save the results
    reg1 = np.int(np.round(reg,4)*10000)
    path_save_BNN_output  = os.path.join(results_dir,'output_autoMPG_l2_'+str(reg1)+'.txt')             
    with open(path_save_BNN_output, "wb") as fp:   #Pickling
