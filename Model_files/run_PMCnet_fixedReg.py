...
def run_PMCnet_fixedReg(simulations,reg,loss,N_resampled, results_dir, x_val, y_val1,is_binary, N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1,epsilon2,logger):
    ...
    for i in range(simulations): 
        ...
        print("samples",len(all_samples_temp[-1][-1]))
        ...
        print("population",output_posterior_val[2])
...
def run_PMCnet_fixedReg_extended(simulations,reg,loss, results_dir, x_val, y_val1,is_binary, N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1,epsilon2,logger):
    tp['regularization_weight'] = reg
    output_vec = []
    output_posterior_val_vec = []
    auc_mean_vec = []
    ...
    for i in range(simulations): 
        ...
        output_posterior_temp = []
        population = all_samples_temp[-1] # the last iteration
        weights = all_weights_temp[-1]    #the last iteration 
        #print(all_samples_temp)
        print(len(all_samples_temp))
        output_posterior_val = BNN_posterior_extended(N, K,population, weights, x_val, y_val1, tp)
        output_posterior_val_vec.append(output_posterior_val)
        auc_mean = np.mean(output_posterior_val[3])
        Accuracy_mean = np.mean(output_posterior_val[-3])
        auc_mean_vec.append(auc_mean)
        Accuracy_mean_vec.append(Accuracy_mean) 
        EV_AUC_vec.append(output_posterior_val[-1]["AUC"])
        EV_accuracy_vec.append(output_posterior_val[-1]["accuracy"])
        print("the accuracy is", output_posterior_val[-1]["accuracy"])
        # save the results
    reg1 = np.int(np.round(reg,4)*10000)
    path_save_BNN_output  = os.path.join(results_dir,'output_Ionosphere_l2_'+str(reg1)+'.txt')             
    with open(path_save_BNN_output, "wb") as fp:   #Pickling
    ...
    if loss == 'AUC':
        # we maximize AUC
        return -np.mean(EV_AUC_vec)
    else:
        # we maximize Accuracy
        return -np.mean(EV_accuracy_vec)  
